...
window.addEventListener("load", () => {
  const viewers = document.querySelectorAll(".model-viewer__canvas");

  viewers.forEach((viewer) => {
    // const canvas = viewer.querySelector('.three-canvas-simple1');
    const modelUrl = viewer.dataset.modelUrl;
    const itemType = viewer.dataset.modelType;
    // データが取得できてるかの確認
    dataCheck(viewer, modelUrl, itemType);

    waitForVisibleSize(viewer, () => {
      if (viewer.dataset.modelInitialized === 'true') return;
      viewer.dataset.modelInitialized = 'true';
      initModel(viewer, modelUrl, itemType);
    });
  })
})

function dataCheck(viewer, modelUrl, itemType) {
  if (!viewer) return;
  if (!modelUrl) {
...
function waitForVisibleSize(viewer, onReady) {
  let isDone = false;
  let frameId = null;

  function hasVisibleSize() {
    return viewer.clientWidth > 0 && viewer.clientHeight > 0;
  }
  ...
  function checkSizeOnNextFrame() {
    ...
    if (retryCount < maxRetryCount) {
      frameId = requestAnimationFrame(checkSizeOnNextFrame);
    }
  }
...
function initModel(viewer, modelUrl, itemType) {
